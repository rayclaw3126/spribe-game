// 下级列表：代理 + 玩家混显。代理行可"进入"下钻；玩家行可对其上下分（额度↔余额兑换）。
...
import PlayerBalanceModal from '../components/PlayerBalanceModal.jsx'
...
export default function DownlineList() {
  ...
  const [balanceModalPlayer, setBalanceModalPlayer] = useState(null)

  // 注意：上下分弹窗必须渲染在 loading/error 分支之外（作为始终挂载的兄弟节点）。
  // 弹窗提交成功后会调用全局 refresh()，refresh 内部会短暂把 loading 置 true，
  // 如果弹窗是嵌套在"loading 为 true 时提前 return"的那棵子树里，会被整体卸载再重新挂载，
  // 弹窗自己的本地状态（如已刷新到的最新余额）就会被重置回初始值，页面上会闪回旧数据。
  return (
    <>
      {loading && <div style={{ color: COLORS.textMuted, fontSize: 14 }}>正在加载下级数据…</div>}
              ...
              },
              {
                ...
                render: (row) =>
                  row.kind === 'agent' ? (
                    ...
                    <button
                      type="button"
                      onClick={() => setBalanceModalPlayer(row)}
                      style={{
                        padding: '5px 12px',
                        fontSize: 12.5,
                        fontWeight: 500,
      ...
      {balanceModalPlayer && (
        <PlayerBalanceModal player={balanceModalPlayer} onClose={() => setBalanceModalPlayer(null)} />
      )}
    </>
  )
}

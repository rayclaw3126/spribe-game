// HatTrick 端到端：进帽子戏法→登录→暂存多注(大+小 互补必中)→后端开奖→3骰停后端点数→中奖高亮→⚖抽屉→移动截图。
...
await page.getByText('帽子戏法', { exact: true }).first().click(); await page.waitForTimeout(600)
await page.screenshot({ path: `${OUT}/hattrick-01-login.png` })
...
await page.getByText('大', { exact: true }).first().click()
await page.getByText('小', { exact: true }).first().click()
...
await page.screenshot({ path: `${OUT}/hattrick-02-betting.png` })

// ---- 4. 等 betting 倒计时走完 → rolling 掷骰 → settled 结算（BETTING_T 24s + ROLLING 7s）----
await page.waitForFunction(() => document.body.innerText.includes('掷骰中'), { timeout: 30000 }).catch(() => {})
await page.waitForTimeout(2500)
await page.screenshot({ path: `${OUT}/hattrick-03-rolling.png` })
await page.waitForFunction(() => /已开奖|已结算|\+\$/.test(document.body.innerText), { timeout: 14000 }).catch(() => {})
await page.waitForTimeout(800)
await page.screenshot({ path: `${OUT}/hattrick-04-result.png` })
const after = await balOf()
console.log('server balance after settle:', after, '(部分赢：命中侧 10×1.96=19.60，净 -0.40/期)')

// ---- 5. ⚖ 可验证公平抽屉 ----
await page.locator('button[title="可验证公平"]').first().click().catch(() => {})
await page.waitForTimeout(700)
await page.screenshot({ path: `${OUT}/hattrick-05-fairness.png` })
...
await page.screenshot({ path: `${OUT}/hattrick-06-mobile.png` })

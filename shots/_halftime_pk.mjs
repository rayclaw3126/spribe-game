// HalfTime 端到端：进中场→登录→暂存多注(大+小 互补必中)→后端开奖→20球停后端 balls→和值/低区对→中奖高亮→⚖抽屉→移动截图。
...
await page.getByText('中场', { exact: true }).first().click(); await page.waitForTimeout(600)
await page.screenshot({ path: `${OUT}/halftime-01-login.png` })
...
await page.locator('.htCell').filter({ hasText: '大' }).first().click()
await page.locator('.htCell').filter({ hasText: '小' }).first().click()
...
await page.screenshot({ path: `${OUT}/halftime-02-betting.png` })

// ---- 4. 等 betting 倒计时走完 → drawing 开奖 → settled 结算（BETTING_T 24s + DRAWING 10s）----
await page.waitForFunction(() => /开奖中|开球中|滚球中/.test(document.body.innerText), { timeout: 30000 }).catch(() => {})
await page.waitForTimeout(4000)
await page.screenshot({ path: `${OUT}/halftime-03-drawing.png` })
await page.waitForFunction(() => /已开奖|已结算|\+\$/.test(document.body.innerText), { timeout: 16000 }).catch(() => {})
await page.waitForTimeout(800)
await page.screenshot({ path: `${OUT}/halftime-04-result.png` })
...
await page.screenshot({ path: `${OUT}/halftime-05-fairness.png` })
...
await page.screenshot({ path: `${OUT}/halftime-06-mobile.png` })

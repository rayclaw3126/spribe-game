// WuXing 端到端：进五行→登录→暂存多注(大+小 互补必中)→后端开奖→20球停后端 balls→龙虎/和值对→中奖高亮→⚖抽屉→移动截图。
...
await page.getByText('五行', { exact: true }).first().click(); await page.waitForTimeout(600)
await page.screenshot({ path: `${OUT}/wuxing-01-login.png` })
...
await page.locator('.wxCell').filter({ hasText: '大' }).first().click()
await page.locator('.wxCell').filter({ hasText: '小' }).first().click()
...
await page.screenshot({ path: `${OUT}/wuxing-02-betting.png` })
...
await page.screenshot({ path: `${OUT}/wuxing-03-drawing.png` })
await page.waitForFunction(() => /已开奖|已结算|开奖 · 本局|\+\$/.test(document.body.innerText), { timeout: 16000 }).catch(() => {})
await page.waitForTimeout(800)
await page.screenshot({ path: `${OUT}/wuxing-04-result.png` })
...
await page.screenshot({ path: `${OUT}/wuxing-05-fairness.png` })
...
await page.screenshot({ path: `${OUT}/wuxing-06-mobile.png` })

// LineUp 端到端：进首发阵容→登录→暂存多注(大+小 互补必中)→后端开奖→25格停后端 grid→行和/总和对→中奖高亮→⚖抽屉→移动截图。
...
await page.getByText('首发阵容', { exact: true }).first().click(); await page.waitForTimeout(600)
await page.screenshot({ path: `${OUT}/lineup-01-login.png` })
...
await page.locator('.luCell').filter({ hasText: '大' }).first().click()
await page.locator('.luCell').filter({ hasText: '小' }).first().click()
...
await page.screenshot({ path: `${OUT}/lineup-02-betting.png` })

// ---- 4. 等 betting 倒计时走完 → drawing 开奖 → settled 结算 ----
await page.waitForFunction(() => document.body.innerText.includes('开奖中'), { timeout: 30000 }).catch(() => {})
await page.waitForTimeout(4000)
await page.screenshot({ path: `${OUT}/lineup-03-drawing.png` })
await page.waitForFunction(() => /已开奖|已结算|本局|\+\$/.test(document.body.innerText), { timeout: 16000 }).catch(() => {})
await page.waitForTimeout(800)
await page.screenshot({ path: `${OUT}/lineup-04-result.png` })
const after = await balOf()
console.log('server balance after settle:', after, '(部分赢：命中侧 10×1.95=19.50)')

// ---- 5. ⚖ 可验证公平抽屉 ----
await page.locator('button[title="可验证公平"]').first().click().catch(() => {})
await page.waitForTimeout(700)
await page.screenshot({ path: `${OUT}/lineup-05-fairness.png` })
...
await page.screenshot({ path: `${OUT}/lineup-06-mobile.png` })

...
// ---------- 引擎：已搬 src/games/markets/rollingball.js（单4 a，多桌共用同一份，保 code-split）----------
// ⚠ 不再 re-export：本页原先 `export function drawThree/hitOf/oddsFor` 是全仓【零外部 import】的
//   （唯一引用是 App.jsx 的 lazy 整页懒加载，取的是默认导出不是引擎）。留着 re-export 只会让本页
//   继续触发 3 条 react-refresh/only-export-components —— 单4 (a) 顺手清的正是这 3 条。
//   将来要用引擎的（多桌件、本地重算）一律 import 'games/markets/rollingball'，别回头找本页要。
import { isRed, round2, GROUPS, COMBO_META, oddsFor, ROAD_VIEWS, roadBeadsOf } from './markets/rollingball'
...
// COMBO_META 已随引擎搬家（markets/rollingball.js），此处回引
const ROWS = [

...
export default function HalfTime({ serverBalance, setServerBalance, playerToken, onLogout, onBack }) {
  ...
  // 逐款不同的部分仍在本文件：上局开奖/路珠 两份 xxxByRoom、E 段追两份、A 段换期清盘、切房演出态清理（见下方 handleRoomSwitch）、舞台 key 挂点。
  const {
  ...
  const roomTabs = renderRoomTabs({ tokens: { sel: HALFTIME.sel, strip: HALFTIME.strip, dim: HALFTIME.dim }, isMobile })

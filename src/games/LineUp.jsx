...
export default function LineUp({ serverBalance, setServerBalance, playerToken, onLogout, onBack }) {
  ...
  const roadPhaseRef = useRef({})   // #Ray 手机路珠相位·按房自持（首灌锚真实序号，live +1，跨零点连续）                   // 珠盘路整局记账去重（按 rnd，防 StrictMode 双调用）
  ...
  const beads = roadWindowN(road, roadPhaseRef.current[selectedRoomKey], { cols: ROAD_COLS, rows: 6 })   // #Ray 进场相位真实化：按真实珠序开窗

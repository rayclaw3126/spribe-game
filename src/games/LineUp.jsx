...
export default function LineUp({ serverBalance, setServerBalance, playerToken, onLogout, onBack }) {
  ...
  useEffect(() => {
    let cancelled = false
    // ⚠ 手机零碰：路珠 state 是【桌手共享】的（手机内联珠格走同一 road），播种会把手机路珠
    //   从种子珠灌成真历史 —— 珠数一变即违反「手机逐字节同基线」。故播种只在 hasRail（≥1280）档进行。
  ...
  const ROAD_COLS = 20
  const beads = road.slice(-MOBILE_ROAD_CAP)   // #47：只喂手机内联珠格（532 行），钉回手机专用容量

...
const genIdemKey = () => (crypto.randomUUID ? crypto.randomUUID() : `lineup-${Date.now()}-${Math.random()}`)
...
export default function LineUp({ serverBalance, setServerBalance, playerToken, onLogout, onBack }) {
  ...
  const pendingDataRef = useRef(null)   // 后端 /lineup/play 返回（settleRound 消费）
  ...
  // 唯一赔付点：读后端 /lineup/play 结算结果（命中/赔付/余额全认后端；无 push 项）
  function settleRound() {
    const r = pendingRef.current
    const data = pendingDataRef.current
    let hits, winTotal
    if (data) {
      // 后端结算：命中高亮 = outcome 非 lose；余额只认 balanceAfter
    ...
    const id = setInterval(async () => {
      ...
      if (ph === 'betting') {
        ...
        if (betsRef.current.size > 0) {
          ...
          try {
            const data = await apiPost('/round/lineup/play', { bets: Object.fromEntries(betsRef.current), idempotencyKey: genIdemKey() })
            pendingDataRef.current = data
            pendingRef.current = deriveRound(data.drawResult.grid)   // ← 后端 25 位（行和/总和/段位按后端算，不本地 drawGrid）
          } catch (e) {
            setNetErr(e.message)
            pendingDataRef.current = null
            pendingRef.current = deriveRound(drawGrid())   // 失败：本地开奖仅显示，注单未扣（暂存不扣钱）
          }

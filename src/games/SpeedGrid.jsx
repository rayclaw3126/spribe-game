...
import { teamCarOf } from './markets-ui/carAssets'   // #Ray：冠军车按队色选（TEAM_CAR，非 %10 取余）
...
export default function SpeedGrid({ serverBalance, setServerBalance, playerToken, onLogout, onBack }) {
  ...
  // #Ray 根治手抄：冠军号方块渲染抽成单一共享工厂 —— drawZone 大牌与顶栏 chip 同调本函数，
  //   只传 w/h/fontSize，字色/底色/边框/圆角/辉光全部继承原版（drawZone 白字+队色底），
  //   一个属性不准自定。禁逐属性手抄第二份。
  const champSquareStyle = ({ w, h, fontSize, teamColor }) => ({
    width: w, height: h, borderRadius: 10,
    background: teamColor,
    border: `2px solid ${DERBY.gold}`,
    boxShadow: '0 0 14px rgba(255,213,79,0.45), inset 0 2px 3px rgba(255,255,255,0.25)',
    color: COLORS.white, fontSize, fontWeight: 900,
    fontFamily: "'Space Grotesk', sans-serif",
    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
  })

  // #Ray 顶栏冠军车 chip（走 GameTopBar 现成 subRow 插槽，本体禁改；照 B3 PK10 冠亚 inline 成例）。
  //   构图：队色号方块(真实 1-24) + 整台冠军车(按队色选，teamCarOf)。压缩到顶栏高。
  //   相位：betting→上局冠军 / drawing→冲线中…(防剧透,无车) / settled→本局定格 / 冷启→待开。
  const chipChamp = settled ? cur : lastChamp
  const chipCarSpec = chipChamp != null ? teamCarOf(chipChamp) : null   // {src,filter}，按队色
  const chipTeam = chipChamp != null ? teamOf(chipChamp) : null            // 队色（蓝1-6/红7-12/金13-18/黑19-24）
  const carHt = isMobile ? 26 : 30   // #Ray：撑满顶栏可用高
  const champChip = drawing ? (
    <span style={{ color: DERBY.orange, fontSize: 11, fontWeight: 900, whiteSpace: 'nowrap' }}>冲线中…</span>
  ) : chipChamp == null ? (
    <span style={{ color: DERBY.dim, fontSize: 11, fontWeight: 900, whiteSpace: 'nowrap' }}>待开</span>
  ) : (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap',
      animation: settled ? 'sgChipPop 0.4s ease-out' : 'none' }}>
      <span style={{ color: settled ? DERBY.gold : DERBY.dim, fontSize: 9, fontWeight: 900, letterSpacing: 0.5 }}>{settled ? '本局' : '上局'}</span>
      {/* #Ray ①：号方块调【同一份】champSquareStyle，只传尺寸——字色/底色/边框/圆角/辉光继承 drawZone */}
      <span data-champ-src="topbar" style={champSquareStyle({ w: carHt, h: carHt, fontSize: isMobile ? 13 : 15, teamColor: chipTeam.c })}>{chipChamp}</span>
      {/* #Ray ②：车图按队色选（17→金/黄车，禁再出绿车）；黑队绿车压暗代黑（PK10 成例） */}
      <img src={chipCarSpec.src} alt="" style={{ height: carHt, width: 'auto', display: 'block', filter: `drop-shadow(0 1px 2px rgba(0,0,0,0.5)) ${chipCarSpec.filter}` }} />
      <style>{`@keyframes sgChipPop{0%{transform:scale(0.7);opacity:0}60%{transform:scale(1.1)}100%{transform:scale(1)}}`}</style>
    </span>
  )
  ...
  const topBar = (
    <>
      <GameTopBar balance={serverBalance ?? 0} venue={G.venue ?? G.displayName}
        ...
        phaseChip={phaseChipNode} subRow={champChip} onBack={onBack} onHowTo={() => setRulesOpen(true)} onHistory={() => setHistoryOpen(true)} onFairness={() => setFairOpen(true)} />
      {roomTabs}
  ...
  const drawZone = (drawing || settled) && cur ? (
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, flex: '0 0 auto' }}>
        ...
        <span data-champ={shownChamp} data-champ-src="drawzone" style={champSquareStyle({ w: isMobile ? 54 : 64, h: isMobile ? 66 : 78, fontSize: isMobile ? 26 : 32, teamColor: champTeam.c })}>{drawing ? '?' : shownChamp}</span>
  ...
  const gameCard = (
    <Panel style={{
      ...
      <SpeedGridRoad history={roadSeeded ? road : EMPTY_ROAD} tab={roadTab} onTab={setRoadTab} isMobile={isMobile} fitWidth={isMobile} phaseN={isMobile ? roadPhaseRef.current[selectedRoomKey] : undefined}
